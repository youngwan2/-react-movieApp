...
const DetailTaps: React.FC<TapsTypes> = ({ id }) => {
    ...
    //API 호출
    const getDetailTapsData = useCallback (async (value:any) =>{
         if(value !=='')
          await baseSet.get(`/3/movie/${id}/${value}?api_key=${API_KEY}&language=en-US&page=1`)
               .then((response)=>{return setGetApiData(response.data)})
               .catch((error)=>{console.log("tapsError:",error)}) 
    ...
    return (
        ...
        {menuCount === 0 && changeTapsData ==='similar' ? <Similar apiData ={getApiData} /> : null}
        {menuCount === 1 && changeTapsData ==='credits' ? <Cast apiData ={getApiData} /> : null}
        {menuCount === 2 && changeTapsData ==='reviews' ? <Review  apiData = {getApiData}/> : null}
